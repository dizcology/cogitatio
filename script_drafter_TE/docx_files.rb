...
class DOCX
 ...
 def expose
  ...
  def extract_os
	...
	#<Tests
	#topline = @doc.at(".//w:body")
	#top = topline.content.strip.split('Lesson')[1]
	#lessontopic = top.split('—')[1]
	#$preamble["topic"] = lessontopic #line1.content.strip.split(':')[1] #extract topic
	#Tests>
    
	lessonheader = @doc.at(".//w:tbl").dup #copying the lesson number and topic table
	
	$preamble['topic'] = lessonheader.rows[0].cells[2].content.strip #extracting lesson topic
	
	@doc.at(".//w:tbl").remove #remove the "Lesson header" table
	@doc.at(".//w:tbl").remove #remove the "Basic information" table
  ...
  def extract_cells1
	...
	#UPDATE THIS PART OF CODE TO EXTRACT PREAMBLE INFO FROM TE LESSON PLAN
    tb0=@doc.at(".//w:tbl")
	...
	#<TE code
	$line1 = @doc.at(".//w:body") #first line in document
    #$preamble["topic"] = line1.content.strip.split(':')[1] #extract topic
	#TE code>
