...
f_OS_template="OS_template_TE" #empty OS template
...
$stage_names=["introduction", "homework", "mental", "warm", "material", "practice", "review", "conclusion", "quiz", "exam", "stage", "chapter"]
...
$resources=["OS_template_TE.docx", "pieces.docx", "responses.csv", "template.docx"]
$RUNPATH=Dir.pwd+"/"
$PATH=$RUNPATH
...
if username=="eabalo" && $debug
  $RUNPATH="C:/Users/eabalo/Desktop/script_drafter_TE/"
  $PATH=$RUNPATH
  Dir.chdir($RUNPATH)
end
...
#if !(Dir.glob($RUNPATH).include?("resources"))
...
begin  
  ...
  #$PATH=$RUNPATH if username=="eabalo" && $debug
  Dir.chdir($PATH) 
  list=Dir.glob("*[0-9][0-9]*.docx") & Dir.glob("*plan*.docx") #TE requires only 2 digits
  #list=Dir.glob("*[0-9][0-9][0-9]*.docx") & Dir.glob("*plan*.docx") #seeks file with 3 digits and the word 'plan' in the file name
  if list==[]
    show("Lesson plan must be in docx format, have a 3 digit lesson number and \"plan\" in its name.", "Lesson plan not found.", $WARNING)
  else
...
$lesson='TE-'+/(\d{2})/.match(f_plan)[1]  #TE lesson requires 2 digits and 'TE-'
...
$chr=nil
$draft_path=$PATH+$lesson+"_script_drafts/"  #path of script drafts

Dir.chdir($PATH) 

fos_name= $lesson+os+ext #OS file name
...
if $os_exists==1
  ...
  if rc==7
    ...
    $os_items.each do |num|
      ...
      ft[num].add(num)
      ft[num].save($draft_path+$lesson+"-"+nm+ext)  #creating script files
